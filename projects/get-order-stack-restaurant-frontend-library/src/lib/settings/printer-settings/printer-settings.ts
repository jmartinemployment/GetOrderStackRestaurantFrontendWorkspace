...
import { Printer, PrinterModel, CloudPrntConfig } from '../../models';

@Component({
  selector: 'get-order-stack-printer-settings',
...
export class PrinterSettings implements OnInit {
  ...
  onFormField(field: string, event: Event): void {
    ...
    switch (field) {
      ...
      case 'ipAddress': this._formIpAddress.set(value); break;
    }
  }
  ...
  onEditField(field: string, event: Event): void {
    const value = (event.target as HTMLInputElement).value;
    switch (field) {
      case 'name': this._editName.set(value); break;
      case 'ipAddress': this._editIpAddress.set(value); break;
    }
  }
